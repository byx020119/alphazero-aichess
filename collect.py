        ...
        for i in range(n_games):
            ...
            if CONFIG['use_redis']:
                ...
                if os.path.exists(CONFIG['train_data_buffer_path']):
                    while True:
                        try:
                            with open(CONFIG['train_data_buffer_path'], 'rb') as data_dict:  # data_dict从硬盘读到内存
                                data_file = pickle.load(data_dict)  # pickle 存储数据的字典
                                self.data_buffer = deque(maxlen=self.buffer_size)
                                self.data_buffer.extend(data_file['data_buffer'])
                                self.iters = data_file['iters']
                                del data_file  # 从硬盘中取到内存中，删除data_file
                                self.iters += 1
                                self.data_buffer.extend(play_data)
                            print('成功载入数据')
                            break
                        except:
                            time.sleep(30)  # 说明被占用，则停止尝试30s
                else:  # 说明是第一次打开
                    self.data_buffer.extend(play_data)
                    self.iters += 1
            data_dict = {'data_buffer': self.data_buffer, 'iters': self.iters}
            with open(CONFIG['train_data_buffer_path'], 'wb') as data_file:
                pickle.dump(data_dict, data_file)  # data_dict从内存写到硬盘
        return self.iters
